    ...
    <el-dialog v-model="detailVisible" title="商家详情" width="600px">
      <el-descriptions :column="1" border v-if="currentMerchant">
        ...
        <el-descriptions-item label="商家类型">
          <el-tag type="primary">酒店商家</el-tag>
...
import { adminGetPendingHotelMerchants, adminApproveMerchant, adminRejectMerchant } from '@/api/admin'
...
const loadList = async () => {
  loading.value = true
  try {
    const res = await adminGetPendingHotelMerchants({ page: page.value, pageSize: pageSize.value })
